...
channel-approval-reply-instruction-yesno = 回复：“{ $token } yes”、“{ $token } no” 或 “{ $token } always”
channel-approval-reply-instruction-approve-deny = 回复 `{ $token } approve` / `{ $token } deny` / `{ $token } always`。
