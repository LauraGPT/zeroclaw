...
channel-approval-reply-instruction-yesno = Reply: "{ $token } yes", "{ $token } no", or "{ $token } always"
channel-approval-reply-instruction-approve-deny = Reply `{ $token } approve` / `{ $token } deny` / `{ $token } always`.
