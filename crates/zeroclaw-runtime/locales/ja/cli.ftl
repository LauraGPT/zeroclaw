...
channel-approval-reply-instruction-yesno = 返信：「{ $token } yes」、「{ $token } no」、または「{ $token } always」
channel-approval-reply-instruction-approve-deny = 「{ $token } approve」/「{ $token } deny」/「{ $token } always」と返信してください。
