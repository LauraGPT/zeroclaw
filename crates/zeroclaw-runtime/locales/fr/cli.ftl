...
channel-approval-reply-instruction-yesno = Répondez : "{ $token } yes", "{ $token } no" ou "{ $token } always"
channel-approval-reply-instruction-approve-deny = Répondez par `{ $token } approve` / `{ $token } deny` / `{ $token } always`.
