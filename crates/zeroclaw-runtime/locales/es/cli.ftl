...
channel-approval-reply-instruction-yesno = Responde: "{ $token } yes", "{ $token } no" o "{ $token } always"
channel-approval-reply-instruction-approve-deny = Responde con `{ $token } approve` / `{ $token } deny` / `{ $token } always`.
